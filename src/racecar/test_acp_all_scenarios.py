...
if __name__ == '__main__':
    ...
    for scenario in ['id', 'vehicles_2', 'vehicles_3', 'vehicles_4', 'vehicles_5']:
        ...
        # ###
        # ###
        # if scenario == 'vehicles_2':
        #     for t in range(stop_pt):
        #         print(t+start_t, np.mean(stl_satisfied_emp_prob[t]))
        # ###
        # ###

        ''' for Lemma 1 '''
        # average across all traces in all seeds
        # coverage_emp_prob = [np.mean(coverage_emp_prob[t]) for t in range(max_t) if len(coverage_emp_prob[t])>0]
        ...
        ax[0].plot(list(range(start_t, len(expanding_ave)+start_t)), expanding_ave, alpha=0.7)

        ''' for Theorem 1 '''
        # average across all traces in all seeds
        # stl_satisfied_emp_prob = [np.mean(stl_satisfied_emp_prob[t]) for t in range(max_t) if len(stl_satisfied_emp_prob[t])>0]
        ...
        label = {'id':'ID', 'vehicles_2':'2 obstacles', 'vehicles_3':'3 obstacles', 'vehicles_4':'4 obstacles', 'vehicles_5':'5 obstacles'}[scenario]
        ax[1].plot(list(range(start_t, len(expanding_ave)+start_t)), expanding_ave, alpha=0.7, label=label)
    ...
    ax[0].axhline(y = 1-delta, color='black', linestyle='--', linewidth=1)
    ...
    ax[1].set_ylim([0.989, 1.001])
    ax[1].set_yticks([0.99,1.0])
