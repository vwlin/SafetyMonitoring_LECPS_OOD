...
from .racetrack_split import get_racetrack_split
...
class data_generator(object):
    ...
    def __init__(self, parser, log, split='train', phase='training'):
        ...
        assert split in ['train', 'val', 'test', 'ood_test'], 'error'
        ...
        if parser.dataset == 'nuscenes_pred':
            ...
            self.init_frame = 0
        elif parser.dataset == 'racetrack':
            print(f'Loading data from {parser.dataset} {parser.dataset_mode}')
            data_root = parser.data_root_racetrack
            seq_train, seq_val, seq_test = get_racetrack_split(parser.dataset, parser.dataset_mode)
            self.init_frame = 0
        else:
