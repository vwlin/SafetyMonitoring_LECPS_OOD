...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_seed', type=int, default=None) # added for testing 10 seeds of failure prediction algorithm
    args = parser.parse_args()

    """ setup """
    cfg = Config(args.cfg, dataset_seed=args.dataset_seed, tmp=args.tmp, create_dirs=True)
    ...
    if args.metrics_file is not None:
        ...
        args.start_epoch = min(metrics, key=lambda x: metrics[x]['ADE'])
        print(f'Epoch {args.start_epoch} selected for finetuning')
