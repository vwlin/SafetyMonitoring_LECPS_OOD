...
from data.racetrack_split import get_racetrack_split
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='racetrack')
    parser.add_argument('--dataset_mode',
                        default='id_s0', type=str, help='type of racetrack dataset. only used if dataset=racetrack. default is id_s0')
    parser.add_argument('--results_dir', default=None)
    parser.add_argument('--data', default='test')
    parser.add_argument('--log_file', default=None)
    parser.add_argument('--metrics_file', default=None) # added for testing 10 seeds of failure prediction algorithm
    parser.add_argument('--epoch', type=int, default=None) # added for testing 10 seeds of failure prediction algorithm
    ...
    elif dataset == 'racetrack':
        gt_dir = f'datasets/{args.dataset}/{args.dataset_mode}'
        seq_train, seq_val, seq_test = get_racetrack_split(args.dataset, args.dataset_mode)
        seq_eval = globals()[f'seq_{args.data}']
    else :                            # ETH/UCY
    ...
    for seq_name in seq_eval:
        ...
        for line_data in gt_data:
            line_data = np.array([line_data.split(' ')])[:, [0, 1, 13, 15]][0].astype('float64')
    ...
    # if testing validation split, keep a record of the metrics evaluated for this epoch
    if args.data == 'val':
        ...
        for name, meter in stats_meter.items():
            ...
            metrics[args.epoch][name] = meter.avg

        with open(args.metrics_file, 'wb') as f:
