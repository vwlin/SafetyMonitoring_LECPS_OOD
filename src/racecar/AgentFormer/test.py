...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_seed', type=int, default=0) # added for testing 10 seeds of failure prediction algorithm
    args = parser.parse_args()

    """ setup """
    cfg = Config(args.cfg, dataset_seed=args.dataset_seed)
    
    if args.epochs is None:
        if args.metrics_file is None:
            epochs = [cfg.get_last_epoch()]
        else:
            # pull validation results and choose epoch with best performance
            with open(args.metrics_file, 'rb') as f:
                metrics = pickle.load(f)
            best_epoch = min(metrics, key=lambda x: metrics[x]['ADE'])
            epochs = [best_epoch]
            print(f'Epoch {best_epoch} selected for testing')
    else:
    ...
    for epoch in epochs:
        ...
        for split in data_splits:
            ...
            metrics_file = os.path.join(cfg.cfg_dir, 'validation_metrics.pkl')
            cmd = f"python3.7 eval.py --dataset {cfg.dataset} --dataset_mode {cfg.dataset_mode} --results_dir {eval_dir} --data {split} --log {log_file} --metrics_file {metrics_file} --epoch {epoch}"
