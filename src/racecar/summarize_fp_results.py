...
if __name__ == '__main__':
    ...
    if args.scenario == 'id':
        config = 'racetrack_agentformer_pre'
    else:
        train_level = {False:'ogtest', True:'fttest'}[args.use_incremental]
        config = f'racetrack_{args.scenario}_{train_level}'
    ...
    for dataset_seed in range(10):
        ...
        metrics_file = f'AgentFormer/results_s{dataset_seed}/{config}/validation_metrics.pkl'
        with open(metrics_file, 'rb') as f:
            metrics = pickle.load(f)
        best_epoch = min(metrics, key=lambda x: metrics[x]['ADE'])
        predictions_base = f'AgentFormer/results_s{dataset_seed}/{config}/results/epoch_{best_epoch:04d}'
        ...
        fp_results_dir = make_fp_results_dir(args.fp_results_root, dataset_seed, args.scenario, args.split,
                                            predictions_base, args.gt_pred, args.use_incremental, args.cluster_method,
